            ...
            {experience.map((exp, index) => (
              <motion.div key={index} className="relative pl-12"
                ...
                <motion.div
                  ...
                  viewport={{ once: true }} transition={{ duration: 0.3, delay: index * 0.1 + 0.2 }}
                ...
                <motion.div
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-4">
                    <div>
                      ...
                      <motion.p className="text-sm text-primary font-medium" whileHover={{ scale: 1.01 }}>
                        {exp.company}
                      </motion.p>
                    </div>
                    <motion.span
                  ...
                  <ul className="space-y-2">
                    {exp.bullets.map((bullet, bulletIndex) => (
                      <motion.li key={bulletIndex}
                        className="text-sm text-muted-foreground leading-relaxed flex items-start gap-2"
                        initial={{ opacity: 0, x: -10 }} whileInView={{ opacity: 1, x: 0 }}
                        viewport={{ once: true }} transition={{ delay: (index * 0.1) + (bulletIndex * 0.05) }}>
                        <motion.div
